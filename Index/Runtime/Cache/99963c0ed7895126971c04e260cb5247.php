...
<div class="good_simple">
    <div class="good_simple_mid">
        <div class="gsm_show">
            ...
            <div class="gsm_right">
                <h3>
                    <?php $db = M("goods");$data = $db->where(array("id"=>14))->select();foreach($data as $k=>$field){ $url = U("Good/index",array('gid'=>$field['id']));?><a ss="{}">
                            <?php echo ($field["name"]); ?>
                        </a><?php } ?>
                </h3>
                <p> <span class="gsm_type">价格</span>
                <?php $db = M("goods");$data = $db->where(array("id"=>14))->select();foreach($data as $k=>$field){ $url = U("Good/index",array('gid'=>$field['id']));?><del ss="{}" style="font-size: 20px;"><?php echo ($field["mprice"]); ?></del><?php } ?>
                <span>元<span></p>
                        <p> <span class="gsm_type">促销</span> <span class="limit_time">限时促销</span>
                        <?php $db = M("goods");$data = $db->where(array("id"=>14))->select();foreach($data as $k=>$field){ $url = U("Good/index",array('gid'=>$field['id']));?><span class="gsm_p" ss="{}"><?php echo ($field["price"]); ?></span><?php } ?>
                        ...
                        <div style="clear: left"></div>
                        <div class="gsm_info">
                            <?php $good_mes_all = get_goods_mes(14);$specs = $good_mes_all["specs"];$specsx = get_spec_name($specs);foreach($specsx as $k=>$field){if($k<20){ ?><div>
                                    <span ss="{}"><?php echo ($field['attr_name']); ?></span>
                                    <ul class="size">
                                        <?php $good_mes_all = get_goods_mes(14);$specs = $good_mes_all["specs"][$field['attr_id']];foreach($specs as $k=>$field){?><li ss='{}'>
                                ...
                                <div id="box1">
                                    ...
                                    <div class="attr">
                                        <ul>
                                            ...
                                            <?php $good_mes_all = get_goods_mes(14);$attrs = $good_mes_all["attrs"];foreach($attrs as $k=>$field){if($k<200){ ?><li ss="{}">
